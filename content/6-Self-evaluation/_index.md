...
**2. Technical direction.** I chose the problem and the stack, and I made the calls the rest of the work depended on: XLM-RoBERTa rather than PhoBERT (for zero-shot English coverage), a self-hosted quantised model rather than a managed inference endpoint (for cost), and — most importantly — the decision to keep the fine-tuned model in production despite it scoring below the TF-IDF baseline on macro-F1, because its recall on OFFENSIVE and HATE was materially higher and a moderation system should fail towards catching too much rather than too little.
...
The other three members handled the areas outside the model track: Quan built the Lambda/API Gateway backend and the AWS account foundation, Duc handled DynamoDB, IAM and ECR, and Quoc built the React front end. My work sat at the seams between all of them, which is where most of the integration bugs surfaced.
...
| 8 | **Teamwork** | Worked closely with Quoc (UI) and Quan & Duc (backend/infra) at integration points | ✅ | ☐ | ☐ |
