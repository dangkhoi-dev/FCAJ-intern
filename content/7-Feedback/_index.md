...
* **Automating the build.** Two reviewers independently pointed at the same weakness: the Lambda image is built by hand on a developer machine and pushed to ECR manually. That is not reproducible, not auditable and does not scale to a team. The replacement is a GitHub Actions workflow triggered on push — pull the model artefacts from S3, `docker build` on the runner, tag with the commit SHA instead of a floating `v1`, push to ECR through an OIDC role so no long-lived AWS keys live in the repository, then `UpdateFunctionCode`. The infrastructure should move to AWS SAM or CDK at the same time. The team already runs GitHub Actions for this report website, so only the Lambda image was left out.
* **Consolidating the edge layer.** CloudFront already fronts the API, which gives Shield protection and TLS termination at the edge — but *not* caching, since `/moderate` is a `POST` with a different body on every call. The remaining step is to route the front end through the **same** distribution (`/api/*` to API Gateway, everything else to the static site). That would place the interface and the API on a single origin, removing the CORS requirement described in section 5.5 rather than configuring around it, and would give the product one custom domain.
